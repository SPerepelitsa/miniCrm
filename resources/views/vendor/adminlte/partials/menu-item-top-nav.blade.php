@if (is_array($item))
    <li class="{{ $item['top_nav_class'] }}">
        ...
        >
            ...
            {{ trans('adminlte::menu.'.$item['text']) }}
            @if (isset($item['label']))
                <span class="label label-{{ isset($item['label_color']) ? $item['label_color'] : 'primary' }}">{{ trans('adminlte::menu.'.$item['label']) }}</span>
                    ...
                    @else
                        <li class="{{ $subitem['top_nav_class'] }}">
                            <a href="{{ $subitem['href'] }}">
                                ...
                                {{ trans('adminlte::menu.'.$subitem['text']) }}
                                @if (isset($subitem['label']))
                                    <span class="label label-{{ isset($subitem['label_color']) ? $subitem['label_color'] : 'primary' }}">{{ trans('adminlte::menu.'.$subitem['label'] }}</span>
                                @endif
                            </a>
                        </li>
                    @endif
